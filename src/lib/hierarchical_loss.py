...
class HierarchicalLoss(object):
    def __init__(self, df_Sc, df_St, seed=0):
        # Create Sc
...
class RandomHierarchicalLoss(object):
    ...
    def objective(self, preds, train_data):
        ...
        if number < (1 / train_data.params['hier_freq']):
            ...
            n_levels_random = self.rng.integers(1, max_levels_random + 1)
            S_aggs_list = []
            for _ in range(n_levels_random):
                n_categories_per_level = self.rng.integers(2, max_categories_per_random_level + 1)
                codes = self.rng.integers(0, n_categories_per_level, size=(n_bottom_timeseries, ))
