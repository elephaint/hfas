...
#%% Create synthetic data with hts
# s, e = datetime(2018, 1, 1), datetime(2023, 1, 1)
# hsd = load_hierarchical_sine_data(s, e).resample('1D').apply(sum)
# data = hsd.drop(columns = ['total', 'a', 'b', 'c', 'a_x', 'a_y', 'b_x', 'b_y', 'c_x', 'c_y'])
# data = data.stack().reset_index()
# data.rename(columns = {'level_0':'date', 'level_1':'item', 0:'target'}, inplace=True)
# data['group'] = data['item'].str[0].astype("category")
# data['subgroup'] = data['item'].str[0:3].astype("category")
# data['item'] = data['item'].astype("category")
# data = data[['date', 'group', 'subgroup', 'item', 'target']]
...
algorithm = 'lightgbm'
dataset = 'synthetic'
experiment = 'hierarchical_se_nonrandom'
experiment_name = f'{algorithm}_{dataset}_{experiment}'
level_weights = List([1, 1, 1, 1])
drop_cols = ['date', 'target']
cat_cols = ['group', 'subgroup', 'item']
level_cols = ['date', 'group', 'subgroup']
#%% Validation loop
forecast_day = 0
metrics = []
...
test_level_cols = ['date', 'group', 'subgroup']
