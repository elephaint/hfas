#%% Read packages
import pandas as pd
from hierts.reconciliation import apply_reconciliation_methods, calc_level_method_rmse, hierarchy_temporal, hierarchy_cross_sectional
from src.exp_m5 import read_m5, create_forecast_set, exp_m5_globalall, exp_m5_sepagg, exp_m5_globalbottomup, exp_m5_globalbottomup_withtemp
...
folder = 'exp1_storeid=0_temporal'
n_seeds = 1
...
for experiment in experiments_global:
    ...
    for seed in range(n_seeds):
        ...
        forecast_seed =  exp_m5_globalall(X, Xind, targets, target, time_index, end_train, df_Sc, df_St, 
                                         exp_name=exp_name, params=params, exp_folder=folder,
                                         seed=seed)
        # Apply reconciliation methods
        forecasts_test = forecast_seed.loc[:, start_test:]
        forecasts_methods = apply_reconciliation_methods(forecasts_test, df_Sc, targets.loc[:, :end_train], forecast_seed.loc[:, :end_train],
...
experiments_bu = [
                    ...
                    'fobj': 'hierarchical_obj_se',
                    'feval': 'hierarchical_eval_hmse_withtemp'},
...
for experiment in experiments_bu:
    ...
    for seed in range(n_seeds):
        ...
        params = default_params.copy()
        forecast_seed =  exp_m5_globalbottomup_withtemp(X, Xind, targets, target, time_index, end_train, 
                                                name_bottom_timeseries, df_Sc, df_St, exp_folder=folder,
